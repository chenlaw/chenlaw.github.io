...
(function () {
  ...
  function loadModelAndTextures(modelId, texturesId) {
    ...
    loadlive2d("live2d", `${apiPath}/get.json?id=${modelId}-${texturesId}`);
  ...
  async function switchModel() {
    ...
    try {
      const result = await fetchJson(`${apiPath}/${endpoint}.json?id=${modelId}`);
  ...
  async function switchTextures() {
    ...
    try {
      const result = await fetchJson(`${apiPath}/${endpoint}.json?id=${modelId}-${texturesId}`);
